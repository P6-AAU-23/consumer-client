...
def test_init_invalid_sensitivity_too_low():
    with pytest.raises(AssertionError):
        ChangeSuppressor(-0.1)

def test_init_invalid_sensitivity_too_high():
    with pytest.raises(AssertionError):
        ChangeSuppressor(1.1)

def test_change_over_threshold_same_dimension():
    # Arrange
...
def test_change_under_threshold_same_dimension():
    # Arrange
    input = np.ones((10, 10, 3), dtype=np.uint8) * 255
    input[3:7, 3:7] = 0
